...
for ikpt = 1:size(NMD.kpt.kpt_poi,1)
    ...
    for i = 1:NMD.NUM_MODES
        ...
        cv_ph = 1;%(x_cv^2)*(exp(x_cv)/((exp(x_cv)-1)^2)); %specific heat per mode divided by Kb
...
plot(1:size(f_SED),f_SED,'pb','MarkerSize',10);
hold on;
...
NMD.grp_vel_vgx_vgy_vgz_Q1 = [sqrt(mean(sorted_data(:,3).^2)),sqrt(mean(sorted_data(:,4).^2)),sqrt(mean(sorted_data(:,5).^2))];
...
%
NMD.kappa.kappax_estm_Q1 = sum(NMD.kappa.kpt_kx_ky_kz_vg_tau_1st(:,2));
NMD.kappa.kappay_estm_Q1 = sum(NMD.kappa.kpt_kx_ky_kz_vg_tau_1st(:,3));
NMD.kappa.kappaz_estm_Q1 = sum(NMD.kappa.kpt_kx_ky_kz_vg_tau_1st(:,4));
[NMD.kappa.kappax_estm_Q1,NMD.kappa.kappay_estm_Q1,NMD.kappa.kappaz_estm_Q1] % in W/mK
