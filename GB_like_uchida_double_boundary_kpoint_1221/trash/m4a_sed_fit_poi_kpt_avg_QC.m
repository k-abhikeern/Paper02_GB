...
%#########################################################
cnt = 1;
for ikpt = 1:size(NMD.kpt.kpt_poi,1)%size(NMD.kptlist(:,1:3,ikslice),1) size(NMD.kpt.symm_points_index,1)
    ...
    for i = 1:NMD.NUM_MODES
        ...
        %                 figure(1);
        %
        %                 %  w_x(1,:) = SED.omega(1,:)*w_LJ2THz;
        %                 %                         plot( SED.omega(1,:),SED.SEDpoi_kpt_avg(ikpt,:,i),'ko',SED.omega(1,:),...
        %                 %                             lor_func([c_fit], SED.omega(1,:)),'b-','DisplayName',kpt);
        %
        %                 %         semilogy( SED.omega(1,:),SED.SEDpoi_kpt_avg(ikpt,:,i),'ko',SED.omega(1,:),...
        %                 %             lor_func([c_fit], SED.omega(1,:)),'b-','DisplayName',kpt);
        %
        %                 %         plot( SED.omega(1,:),SED.SEDpoi_kpt_avg(ikpt,:,i),'ob');
        %                 %         hold on;
        ...
        %
        %                 figure(2);
        %                 plot(SED.omega(wleft:wright),residual,'ok');
        %                 xlabel('omega','fontsize',12);
...
loglog(omega,tau,"rs");%,'MarkerSize',10,'MarkerFaceColor','g','MarkerEdgeColor','g');
hold on;
